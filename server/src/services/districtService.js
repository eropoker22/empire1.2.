...
async function ensureDistricts(gameMode = "war", serverKey = "") {
  ...
  const existing = await pool.query(
    "SELECT COUNT(*) FROM districts WHERE game_mode = $1 AND server_key = $2",
    [mode, resolvedServerKey]
  );
  const count = Number(existing.rows[0].count);
  if (count > 0) return;

  const seed = `${modeConfig.mapSeed}:${resolvedServerKey}`;
  ...
  city.districts.forEach((district) => {
    params.push(district.name, district.type, JSON.stringify(district.polygon), district.income, district.influence, mode, resolvedServerKey);
    values.push(`($${idx}, $${idx + 1}, $${idx + 2}, $${idx + 3}, $${idx + 4}, $${idx + 5}, $${idx + 6})`);
    idx += 7;
  });

  await pool.query(
    `INSERT INTO districts (name, type, polygon, base_income, influence_level, game_mode, server_key)
     VALUES ${values.join(",")}`,
    params
  );
}
...
async function listDistricts(gameMode = "war", serverKey = "") {
  const mode = normalizeGameMode(gameMode);
  const resolvedServerKey = normalizeServerKey(mode, serverKey);
  await ensureDistrictDestructionSchema();
  await ensureDistricts(mode, resolvedServerKey);
