document.addEventListener("DOMContentLoaded", () => {
  ...
  grid.querySelectorAll(".structure-card").forEach((card) => {
    card.addEventListener("click", () => {
      grid.querySelectorAll(".structure-card").forEach((btn) => {
        btn.classList.remove("structure-card--active");
      });
      card.classList.add("structure-card--active");
      const choice = card.dataset.structure;
  ...
  if (avatarLeft && avatarRight && avatarGrid) {
    ...
    avatarLeft.addEventListener("touchstart", (e) => {
      e.preventDefault();
      startHold(-1);
    }, { passive: false });
    avatarRight.addEventListener("touchstart", (e) => {
      e.preventDefault();
      startHold(1);
    }, { passive: false });
  ...
  if (selectedStructure) {
    const selectedButton = grid.querySelector(`.structure-card[data-structure="${selectedStructure}"]`);
    if (selectedButton) selectedButton.classList.add("structure-card--active");
  }
